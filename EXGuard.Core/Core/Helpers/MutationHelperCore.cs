        ...
        internal static void InjectKey_Int(MethodDef method, int keyId, int key)
        {
            foreach (Instruction instr in method.Body.Instructions)
            {
                if (instr.OpCode == OpCodes.Ldsfld)
                {
                    ...
                    int _keyId;
                    if (field.DeclaringType.FullName == "Mutation" && Field2IntIndex.TryGetValue(field.Name, out _keyId) && _keyId == keyId)
                    {
        ...
        internal static void InjectKey_Long(MethodDef method, int keyId, long key) {
			foreach (Instruction instr in method.Body.Instructions)
            {
                if (instr.OpCode == OpCodes.Ldsfld)
                {
                    ...
                    int _keyId;
                    if (field.DeclaringType.FullName == "Mutation" && Field2LongIndex.TryGetValue(field.Name, out _keyId) && _keyId == keyId)
                    {
        ...
        internal static void InjectKey_ULong(MethodDef method, int keyId, ulong key)
        {
            foreach (Instruction instr in method.Body.Instructions)
            {
                if (instr.OpCode == OpCodes.Ldsfld)
                {
                    ...
                    int _keyId;
                    if (field.DeclaringType.FullName == "Mutation" && Field2ULongIndex.TryGetValue(field.Name, out _keyId) && _keyId == keyId)
                    {
                        instr.OpCode = OpCodes.Ldc_I8;
        ...
        internal static void InjectKey_String(MethodDef method, int keyId, string key)
        {
            foreach (Instruction instr in method.Body.Instructions)
            {
                if (instr.OpCode == OpCodes.Ldsfld)
                {
                    ...
                    string _keyId;
                    if (field.DeclaringType.FullName == "Mutation" && Field2LdstrIndex.TryGetValue(field.Name, out _keyId) && _keyId == keyId.ToString())
                    {
        ...
        internal static void InjectKeys_Int(MethodDef method, int[] keyIds, int[] keys)
        {
            foreach (Instruction instr in method.Body.Instructions)
            {
                if (instr.OpCode == OpCodes.Ldsfld)
                {
                    var field = instr.Operand as IField;
                    int _keyIndex;
                    if (field.DeclaringType.FullName == "Mutation" &&
        ...
        internal static void ReplaceValue_T(MethodDef method, Instruction ret_inst)
        {
            for (int i = 0; i < method.Body.Instructions.Count; i++)
            {
                ...
                if (instr.OpCode == OpCodes.Call &&
                    md.DeclaringType.Name == "Mutation" &&
                    md.Name == "Value")
                {
        ...
        internal static void ReplacePlaceholder(MethodDef method, Func<Instruction[], Instruction[]> repl) {
			...
			for (int i = 0; i < method.Body.Instructions.Count; i++) {
				...
				if (instr.OpCode == OpCodes.Call) {
					...
					if (operand.DeclaringType.FullName == "Mutation" &&
